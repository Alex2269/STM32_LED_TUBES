...
void MAX7219_init(uint8_t pinCLK, uint8_t pinDIN, uint8_t pinCS)
{
  ...
  HAL_GPIO_WritePin(PORT, PIN_CLK, LOW);
  HAL_GPIO_WritePin(PORT, PIN_DIN, LOW);
  HAL_GPIO_WritePin(PORT, PIN_CS, HIGH);
}
...
void shift_Out(uint8_t val)
{
  ...
  for (i = 0; i < 8; i++)
  {
    ...
    if(not_null)
    {
      HAL_GPIO_WritePin(PORT, PIN_DIN, HIGH);
    }
    else
    {
      HAL_GPIO_WritePin(PORT, PIN_DIN, LOW);
    }
    HAL_GPIO_WritePin(PORT, PIN_CLK, HIGH);
    HAL_GPIO_WritePin(PORT, PIN_CLK, LOW);
  }
}
...
{
  HAL_GPIO_WritePin(PORT, PIN_CS, LOW);
  shift_Out (addr);
  shift_Out (data);
  HAL_GPIO_WritePin(PORT, PIN_CS, HIGH);
}
