...
void MAX7219_init(void)
{
  ...
  LL_GPIO_ResetOutputPin(CLK_7219_GPIO_Port, CLK_7219_Pin);
  LL_GPIO_ResetOutputPin(DIN_7219_GPIO_Port, DIN_7219_Pin);
  LL_GPIO_SetOutputPin(CS_7219_GPIO_Port, CS_7219_Pin);
}
...
void shift_Out(uint8_t val)
{
  ...
  for (i = 0; i < 8; i++)
  {
    ...
    if(not_null)
    {
      LL_GPIO_SetOutputPin(DIN_7219_GPIO_Port, DIN_7219_Pin);
    }
    else
    {
      LL_GPIO_ResetOutputPin(DIN_7219_GPIO_Port, DIN_7219_Pin);
    }
    LL_GPIO_SetOutputPin(CLK_7219_GPIO_Port, CLK_7219_Pin);
    LL_GPIO_ResetOutputPin(CLK_7219_GPIO_Port, CLK_7219_Pin);
  }
}
...
{
  LL_GPIO_ResetOutputPin(CS_7219_GPIO_Port, CS_7219_Pin);
  shift_Out (addr);
  shift_Out (data);
  LL_GPIO_SetOutputPin(CS_7219_GPIO_Port, CS_7219_Pin);
}
